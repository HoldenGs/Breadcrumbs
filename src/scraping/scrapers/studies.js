...
const majorsListStartText =
	'brings together perspectives from many liberal arts'
const majorsListEndText =
	'<h2 class="visually-hidden" id="sidebar-menu">Sidemenu navigation</h2>'
...
function getStudies() {
	return new Promise((resolve, reject) => {
		Promise.all([getMajors(), getMinors()])
			.then(([majors, minors]) => {
...
function getMajors() {
	return new Promise((resolve, reject) => {
		axios
			.get(majorsAddend)
			.then(({ data }) => {
				const majorTexts = retrieveLinkNames(
					data,
					majorsListStartText,
					majorsListEndText
				)
				...
				majorTexts.forEach((major) => {
					// remove (B.A/S/M.) from end, remove non-matches.
...
function getMinors() {
	return new Promise((resolve, reject) => {
		axios
			.get(minorsAddend)
			.then(({ data }) => {
				const minors = retrieveLinkNames(
					data,
					minorsListStartText,
					minorsListEndText
				)
...
function retrieveLinkNames(data, startText, endText) {
	...
	const text = [...matches].map((match) =>
		match[1].replace(weirdCharRegex, ' ')
	)
