...
const defaultConfig = {
	scrape: {
		// items to scrape and (over)write to JSON.
		...
		classes: {
			// whether to scrape classes (make "false" instead of an object to disable); options as follows
	...
	upload: {
		// items to read from JSON and upload to Firestore (no longer uploading! for now.)
		studies: false,
		departments: false,
		quarterDates: false,
		classes: {
...
async function scrape() {
	...
	if (scrape.studies) await scrapeStudies()

	if (scrape.departments) await scrapeDepartments()

	if (scrape.classes) await scrapeClasses()
}
...
async function scrapeStudies() {
	...
	try {
		if (!fs.existsSync(studiesPath)) fs.mkdirSync(studiesPath)
...
async function scrapeClasses() {
	...
	try {
		if (!fs.existsSync(classesPath)) fs.mkdirSync(classesPath)

		if (!fs.existsSync(termPath)) fs.mkdirSync(termPath)

		let departments = []

		if (department) departments = [department]
		else departments = JSON.parse(await fsp.readFile(departmentsPath, 'utf-8'))
		...
		for (let i = 0; i < departments.length; ++i) {
			...
			await fsp.writeFile(
				path.join(termPath, department + '.json'),
...
function printConfigTasks() {
	console.log(
		'With the currently loaded config, the following tasks will be performed:\n'
	)
	...
	if (!scrape || (!scrape.studies && !scrape.departments && !scrape.classes))
		...
		console.log(
			'The following will be scraped from the UCLA registrar and stored in JSON:'
		)

		if (scrape.studies)
			console.log(' - The list of studies (majors and minors)')

		if (scrape.departments) console.log(' - The list of departments')

		if (scrape.classes) {
			console.log(` - Classes for the ${scrape.classes.term} quarter`)
			console.log(
				`   - With a ${scrape.classes.delay || 5000} ms delay between scrapes`
			)
			if (scrape.classes.department)
				console.log(`   - In the ${scrape.classes.department} department only`)
			else console.log('   - In all departments')
		}
	}
	...
	if (
		...
		if (upload.departments) console.log(' - The list of departments')

		if (upload.quarterDates)
			console.log(
				' - The list of quarters (long names) and their corresponding start dates'
			)
		...
		if (upload.classes) {
			...
			else console.log(' - For all departments')
		}
	}
...
function affirmAndWait(seconds) {
	return new Promise((resolve) => {
		console.log(
			`Are you sure you would like to do all this? Tasks will begin in ${seconds} seconds if the program is not terminated.`
		)
...
function validateConfig() {
	...
	if (scrape.classes) {
		...
		if (
			!scrape.classes.department &&
			!scrape.departments &&
			!fs.existsSync(departmentsPath)
		)
			exitWithError(
				"Trying to scrape classes from all departments, but you haven't scraped all departments yet!"
			)
	}
}
